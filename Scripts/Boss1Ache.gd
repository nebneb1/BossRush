...
const IDLE_TIME = [1.0, 2.0]
const SURROUND_ATTACK_TIME = [10.0, 15.0]
const ATTACKS = [State.SURROUND]
...
func set_state(to : State):
	...
	match current_state:
		State.IDLE:
			play_animation("Idle")
			await get_tree().create_timer(random_val(IDLE_TIME)).timeout
			set_state(ATTACKS.pick_random()) # TODO : Change to pick random attack that wasnt the last one used
